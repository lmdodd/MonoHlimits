...
void makeLTauStack(TString name,TString file,TString dir,int s,TString labelX,TString units = "GeV",bool left=false,TString channel = "",TString json = "Golden",bool log = false,bool dndm=false,bool doRatio = false,double err=0.2)
{
	...
	lumi_sqrtS = "13 TeV";
	if (json=="postfit") lumi_13TeV = channel+" postfit  2016, 35.9 fb^{-1}";
	if (json=="prefit") lumi_13TeV = channel+" prefit  2016, 35.9 fb^{-1}";
    ...
    TH1F * ZVV=0;
    if(f->Get(dir+"/ZVV")!=0 && channel == "#tau_{h}#tau_{h}"){
        ZVV = (TH1F*)(f->Get(dir+"/ZVV"));
    ...
    TH1F* errorBand = (TH1F*)(f->Get(dir+"/TotalBkg"));
    ...
    if(doRatio){
        ...
        TH1F * mc = (TH1F*)(errorBand->Clone());
        TH1F * mcErr = (TH1F*)(errorBand->Clone());
        ...
        data2->GetYaxis()->SetRangeUser(0.2,1.8);
    ...
    //if(channel == "#tau_{e}#tau_{h}") l->AddEntry(ZVV,"Z#rightarrowee","F");
    //if(channel == "#tau_{#mu}#tau_{h}") l->AddEntry(ZVV,"Z#rightarrow#mu#mu","F");
    if(channel == "#tau_{h}#tau_{h}")
